import{c as e,l as t,n,o as r}from"./apiClient-CzlMaPmi.js";var i=`/semester-project-2/`.replace(/\/$/,``);function a(){document.body.insertAdjacentHTML(`afterbegin`,`
  ...
  `);let e=window.location.pathname.replace(i,``)||`/index.html`;document.querySelectorAll(`#nav-mobile-menu a, #nav-desktop a, #desktop-dropdown a`).forEach(t=>{new URL(t.href).pathname.replace(i,``)===e&&(t.closest(`#nav-desktop`)?t.classList.add(`border-b-2`,`border-teal-600`,`bg-gray-100`):t.closest(`#desktop-dropdown`)?t.classList.add(`border-l-8`,`border-teal-600`,`bg-gray-200`):t.classList.add(`border-l-4`,`border-teal-600`,`bg-gray-100`))})}a();var o=`/semester-project-2/`.replace(/\/$/,``),s=document.getElementById(`mobile-menu-profile`),c=document.getElementById(`credits-mobile-menu`),l=document.getElementById(`credits-desktop`),u=document.getElementById(`desktop-dropdown-profile`);function d(){e(),t(),window.location.href=`${o}/pages/login.html`}var f=document.getElementById(`logout`);f&&f.addEventListener(`click`,d);var p=document.getElementById(`logout-desktop`);p&&p.addEventListener(`click`,d);async function m(){try{let e=localStorage.getItem(`user`),t=JSON.parse(e).name,r=(await n(`auction/profiles/${t}`)).data,i=r.name,a=r.email,o=r.credits,d=document.createElement(`img`);d.src=r.avatar.url,d.alt=`${i}'s avatar`,d.className=`w-14 h-14 rounded-full`;let f=document.createElement(`div`),p=document.createElement(`p`),m=document.createElement(`p`);p.className=`text-lg`,m.className=`text-xs`,p.textContent=i,m.textContent=a,f.appendChild(p),f.appendChild(m),s.appendChild(d),s.appendChild(f);let h=document.createElement(`img`);h.src=r.avatar.url,h.alt=`${i}'s avatar`,h.className=`w-14 h-14 rounded-full`;let g=document.createElement(`div`),_=document.createElement(`p`),v=document.createElement(`p`);_.className=`text-lg`,v.className=`text-xs`,_.textContent=i,v.textContent=a,g.appendChild(_),g.appendChild(v),u.appendChild(g),u.appendChild(h),c.innerHTML=`<i class="fa-regular fa-coins text-teal-500 text-xl"></i>${o}`,l.className=`px-2 py-4 border border-teal-600 bg-teal-600 text-white`,l.innerHTML=`<i class="fa-regular fa-coins mr-2"></i>${o}`}catch(e){console.error(e);let t=document.createElement(`p`);t.textContent=`Could not load profile.`,t.className=`text-red-500 text-sm`,s.appendChild(t),u.appendChild(t)}}var h=document.getElementById(`nav-mobile-toggle`),g=document.getElementById(`nav-mobile-menu`),_=h.querySelector(`i`);h.addEventListener(`click`,()=>{let e=g.classList.toggle(`hidden`);_.classList.toggle(`fa-angle-down`,e),_.classList.toggle(`fa-angle-up`,!e),h.setAttribute(`aria-expanded`,!e)});var v=document.getElementById(`desktop-profile-toggle`),y=document.getElementById(`desktop-dropdown`);if(!r())v.innerHTML=`<a
      ...
      >
        ...
        <div class="p-8 flex flex-col gap-2 font-light">
          ...
          <a href="/semester-project-2/pages/info/how-it-works.html" class="hover:text-white">How it works</a>
          <a href="/semester-project-2/pages/profile.html" class="hover:text-white">My Bids</a>
          <a href="/semester-project-2/pages/info/about-us.html" class="hover:text-white">About Us</a>
